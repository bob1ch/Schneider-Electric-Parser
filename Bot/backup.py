...
def handle_message(message):
    ...
    for i in range(len(key_category)):
        ...
        if message.text == key_category[i]:
            ...
            bot.send_message(message.chat.id, 'Отправьте ключевое слово, название продуктиа или номер файла')
    schet = 0
    ...
    if schet == 0:
        ...
        if len(heads_list) > 0:
            ...
            #bot.send_message(message.chat.id, str_heads_links)
        
    if choosed_category == 1 :
        for i in range(len(key_category)):
            if message.text != key_category[i]:
                key_category.index(message.text)
                r = requests.get(dict_category[key_category[i]].replace("?sortByField=Popularity","?langFilterDisabled=true&keyword="+message.text))
                soup = bs(r.text, 'lxml')
                heads = soup.find_all('h5')
                links = soup.find_all('li', class_='list-option-doctype')
                heads_list = []
...
def handle_regex(message):
    ...
    j = 0
    for j in range(len(key_category[j])):
        if message.text == key_category[j]:
            schet = schet +1
        print(schet)
    if schet == 0:
        print(schet)
